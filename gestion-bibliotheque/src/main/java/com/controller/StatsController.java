...
public class StatsController {
    ...
    public void chargerStats() {
        try {
            ...
            long enCours = emprunts.stream()
                .filter(e -> "En cours".equals(e.getStatut())).count();
            long rendus = emprunts.stream()
                .filter(e -> "Rendu".equals(e.getStatut())).count();
            long indisponibles = livres.stream()
                .filter(l -> !l.isDisponible()).count();
            ...
            for (Livre l : livres) {
                String cat = l.getCategorie() == null ? "Non définie" : l.getCategorie();
